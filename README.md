...
```
sudo dnf install gd2gs-0.4.0-1.fc37.noarch.rpm
...
```
python -m pip install gd2gs-0.4.0.tar.gz
```
...
| `HEADER_OFFSET`  | The first row of the Google spreadsheet is expected to be the header. In this case, `HEADER_OFFSET` is 0, which is the default value. If the header is larger, `HEADER_OFFSET` defines the value. If can be defined either globally or specifically for each sheet. |
